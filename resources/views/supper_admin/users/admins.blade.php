                                ...
                                <div class="table-responsive">
                                    <table id="" class="export-table table table-bordered mb-0">
                                        ...
                                        <tr>
                                            ...
                                            @if($title=="Vendors")
                                            <td>{{$p->vendor_amount}} PKR</td>
                                            @endif
                                            <td>{{($p->is_admin==1) ? "Admin": ($p->is_admin==2) ? "Vendor": 'Customer'}}</td>
                                            <td>{{date('d-M-Y',strtotime($p->created_at))}}</td>
                                            <td>
                                                <?php if($p->is_active == 1):?>
                                                <p class="badge badge-pill badge-success" style="vertical-align: middle;font-size: 15px;font-weight: 500;cursor: pointer;" id="{{$p->id}}" table="users" tbl_field="is_active" status="0" operation="Block" @if($p->id == auth()->user()->id) onclick="alert('You cannot block yourself')" @else onclick="change_status(this);" @endif>Activated</p>
                                                @else
